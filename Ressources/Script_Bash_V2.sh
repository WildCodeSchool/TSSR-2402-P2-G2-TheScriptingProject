...
info_modification() {
    ...
    if ssh $nom_distant@$ip_distante cat /etc/passwd | grep $user_inf >/dev/null; then
        ...
        echo ""
        echo "Les données sont enregistrées dans le fichier" "$path_info_file""Info_""$user_inf""_$(date +%Y-%m-%d).txt" && sleep 3s
    else
        # si non -> sortie du script
        echo "L'utilisateur $user_mpd n'existe pas" && sleep 2s
    fi
...
StatusPare_feu() {
    ...
    if [ "$FireW" = "O" ]; then
        ...
        clear
        echo "Mauvais choix - Retour au menu précédent"1
        sleep 2s
        return
    fi
}
